...
但仍是纯 Python 近似（椭圆光瞳、目录外玻璃用 Cauchy 色散），边沿上会差几个千分点，最终以真追迹为准。
...
- **光阑面不再参与挡光**：孔径类型已经定义了光阑大小（Zemax 开 Real 瞄准时 Py=±1 落在光阑**实半径** R 上，
  见下一节）；光阑再拿专利有効径去挡，近距结构会凭空写出 VCY≈0.017 的轴上假渐晕（本篇 15.51 > 15.405）。
- **vignet.py 与 Zemax 的瞄准模型不同 → 光瞳坐标改成「光阑参考」**。Zemax RAIM Real 的 Py=±1 是
  **实光线打在光阑上 主光线高度 ± R**，旧脚本用的是入瞳上的 yep±rEP，差的是光瞳像差。
  （这里原先写的是「± 光阑近轴半径」—— **错的**，2026-09-25 用 ZOS-API 实测推翻，见下一节。）
  现在孔径模型启用时逐视场二分出 Py=±1 对应的入瞳高度当搜索边界，VDY/VCY 折回光阑坐标，
  VCX 用斜光线二维瞄准到光阑 (t·R, 光瞳中心 y) 求（追不出来的那侧只许朝目标方向线性外推）。
...
### Real 瞄准的光阑半径 —— 不是近轴半径（2026-09-25 ZOS-API 实测）

孔径类型 Paraxial Working F/#、`RAIM` Real 时，OpticStudio 2024 R2 的规则（TS-E24 II 与 RF5.2 鱼眼逐位对上）：

1. **入瞳**：主波长近轴追迹，边缘光线像方斜率 = 1/(2·APER) → EPD（= `EPDI`）。
2. **光阑实半径 R** = 主波长**轴上实光线瞄近轴入瞳边缘**时在光阑上的高度：
   ∞ 物 = 入射高 EPD/2 的平行光（面1 上的 `REAY` 正好 = EPD/2）；有限共轭 = 轴上物点 → 近轴入瞳面上高 EPD/2 那一点的直线（tan，不是 sin）。
3. 之后**每个视场、每个波长**的 (Px,Py) 都线性落在光阑上：光阑坐标 = 主光线 + (Px,Py)·R（Py=0.5 → 0.5R；F/g 线的 Py=1 也是 R）。

| 镜头 | 光阑近轴半径（`PARY`） | Real 瞄准的 R（`REAY`） | 专利印的光阑有効径/2 |
|---|---|---|---|
| TS-E24 II（F3.56） | 9.794 | **10.061**（+2.7%；MFD 10.117） | 10.045 |
| RF5.2 鱼眼（F2.91） | 3.977 | **4.020** | — |

入瞳有球差时 R 比近轴半径大。旧 vignet 反过来把入瞳缩到「实光线落在近轴半径上」（打印的「实光线瞄准缩放 ×0.975」），
轴上底线和光瞳都小一圈：TS-E24 的 `axial_3d` 说全过，OpticStudio 面14~22 切轴上光束。

`vignet.py` 现在的模型（探针脚本在 `E:\Download\patent_specs\raim\`：`probe.ps1` / `axfull.ps1` / `index.ps1`）：
- 追迹用 .zmx 的**主波长**（默认 e 线；只用 nd 时 TS-E24 光阑近轴半径就差 0.2%：9.7733 vs 9.7937）。
  折射率：目录牌号读 AGF 色散公式；Offset 解 = 基准色散按 (nd−1)/νd 缩放（对 OpticStudio `INDX`：e 线 ≤1.5e-6、g 线 ≤5e-5）；
  目录外按 nd/νd/dPgF 三项 Cauchy。DOE 偏折按 λ/λ0 缩放。
- 逐结构按该结构的 WFNO 在主波长重算入瞳，R 按上面第 2 条；`axial_3d` = **全部系统波长**各自瞄到 R、整条子午光瞳的逐面包络
  （追迹窗口放宽到 1.5×口径，口径已经切进光束时也量得出来）。
- Py：逐视场找光阑上 主光线 ± R 对应的入瞳高度，扫描范围逐级放宽到 8×rEP
  （TS-E24 54° 的 Py=±1 离主光线 2~3 个 rEP，旧版只扫 1.6 个，扫不到就**静默退回入瞳坐标**，把 Py≈−0.33 当 −1：VCY 0.21 vs 0.73）。
- Px：**二维瞄准**（Newton 解入瞳 (px, ye)，使光阑落点正好是 (t·R, 光瞳中心 y)），不再固定入瞳 y 估光阑 x。

回归（口径全部固定；旧 = 改前 HEAD）：

| spec | CheckOnly ±1 被挡格 旧 → 新 | `axial_3d` 比 OpticStudio 低估 旧 → 新 (mm) |
|---|---|---|
| TS-E24 II | 20/24 → **0** | 0.350 → 0.0004 |
| RF100 微距 | 23/48 → **0** | 0.364 → 0.0007 |
| RF800 | 17/24 → **0** | 1.783 → 0（高估 ≤0.007） |
| RF600 F11（DOE） | 0/24 → 0 | 0.387 → 0.0024 |
| RF35 F1.4 | 23/24 → **0** | 0.623 → 0.0004 |
| RF50 F1.4 | 12/24 → **0** | 0.465 → 0.0006 |
| RF16 | 12/24 → **0** | 0.218 → 0.0005 |
| JP2023-140823A Ex6 | 6/24 → **0** | 0.012 → 0.0005 |
| RF1200 | 17/24 → **0** | 1.954 → 0.0017 |
| Z 24-70 f/2.8 S II（变焦） | 15/36 → **0** | 0.284 → 0.0006 |

对 OpticStudio 自己的 Set Vignetting：TS-E24 VCY rms 0.283 → 0.005；全部 10 个 spec VDY rms ≤0.004、VCY ≤0.010、VCX ≤0.013
（剩下的差是 vignet 的 1% 余量，以及 Set Vignetting 在 ±1 光线其实过得去时也给一点 VCX）。

⚠ **按旧底线定的口径会切轴上光束**：新版在现有 final spec 上报出 RF100 面3/4/5/17~20（最多 −0.037）、
RF1200 面15/16（−0.111）、RF50 F1.4 面11/12（−0.306）。2026-09-25 之前经 apcap 交付的镜头都该用新版 vignet 重跑一遍轴上体检。
⚠ 拿 `axial.ps1` 这类探针当真值时要**先把渐晕系数清零**：轴上视场带 VCY 时 OpticStudio 的 Py=1 已被缩过（RF35 一度对出 −1.2mm 的假差）。
...
| `vignet.py` | 实光线瞄准缩放的括号收缩 `while hi > rEP: hi=(hi+rEP)/2` 在 rEP 尾数为奇时浮点卡死（>2 万次追迹） | 改成 40 次有界循环（2026-09-25 整段删掉：缩放本身是错的，见下一行） |
| **`vignet.py`** | **把 Real 瞄准的光阑半径当成近轴半径**，入瞳反缩 ×0.975：`axial_3d` 普遍低估 0.2~1.9mm，TS-E24 面14~22 在 OpticStudio 切轴上光束；只按 nd 追迹（.zmx 主波长是 e 线） | R = 主波长轴上实光线瞄近轴入瞳边缘时的光阑高度；主波长追迹、全波长轴上包络（见「Real 瞄准的光阑半径」） |
| **`vignet.py`** | 光阑参考坐标的 Py 边界只扫 1.6×rEP，扫不到**静默退回入瞳坐标**：TS-E24 54° 视场 VCY 0.21（OpticStudio 0.73），面1 挡光完全没看见 | 扫描范围逐级放宽到 8×rEP；卡光面统计也改按光阑坐标的 ±1 范围扫 |
| `vignet.py` | VCX 固定入瞳 y、用 sqrt(r²−y²) 估光阑 x，斜光线在光阑上的 y 会漂 → VCX 偏小，回归 6 格 Px±1 被挡 | 二维 Newton 瞄准；门槛取 1e-7·R（RF800 求交噪声 ~1e-6mm，绝对门槛收不住会被当成挡光，VCX 假到 0.55） |
| `vignet.py` | `trace(apert=False)` 仍按 1.05×口径开求交窗口：口径已切进光束时轴上底线量不出来（RF35 MFD g 线面12 需 16.994，窗口 16.985），迭代静默退回主波长入瞳 | 轴上包络用 `wide=True`（1.5×口径+1）；迭代失败回退到上一个好点 |
